...
export default async function handler(req, res) {
  ...
  try {
    const targetUid = process.env.VITE_FIREBASE_USER_ID || 'NpFFvozZSFWnCKdmutkISEGPf8o2';
    const customToken = await getAuth().createCustomToken(targetUid);
    return res.status(200).json({ token: customToken, uid: targetUid });
  } catch (error) {
    console.error('Failed to create Firebase custom token:', error);
    return res.status(500).json({ error: error instanceof Error ? error.message : 'Auth token generation failed' });
  }
}
